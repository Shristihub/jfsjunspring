...
//		return books.stream().sorted((b1,b2)->b1.getTitle().compareTo(b2.getTitle())).toList();
	...
	public List<Book> getByCatAuth(String category, String author) {
		List<Book> books = bookRepository.findByCatAuth(category, author);
		if(books.isEmpty())
			throw new BookNotFoundException("book with this author and category not found");
		return books.stream().sorted(Comparator.comparing(Book::getTitle)).toList();
	}

	@Override
	public List<Book> getByTitleAndCategory(String title, String category) {
		List<Book> books = bookRepository.readByTitleAndCat(title, category);
		if(books.isEmpty())
			throw new BookNotFoundException("book with this title and category not found");
		return books.stream().sorted(Comparator.comparing(Book::getTitle)).toList();
	}

	@Override
	public List<Book> getByTitle(String title) {
		List<Book> books = bookRepository.getAllByTitle(title);
		if(books.isEmpty())
